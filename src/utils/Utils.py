...
class EarlyStopping:
    ...
    This class monitors the validation loss during training and determines whether training should be
    stopped early if the validation loss stops reducing based on the provided parameters.

    Parameters
    ----------
    patience : int, optional
        The number of consecutive times the validation loss can stop improving before triggering early stopping.
        Default is 5.
    min_delta : float, optional
        The minimum change in validation loss that is considered an improvement. If the change is less than
        this value, it is not considered an improvement. Default is 0.025.
    ...
    patience : int
        The number of consecutive times the validation loss can stop improving before triggering early stopping.
    min_delta : float
        The minimum change in validation loss that is considered an improvement.
    counter : int
        A counter that keeps track of how many times the validation loss has not improved.
    best_loss : float or None
        The best validation loss observed so far.
    early_stop : bool
        A flag indicating whether early stopping should be triggered.

    Methods
    -------
    __call__(self, val_loss)
        Call method to update the early stopping based on the provided validation loss.

    Notes
    -----
    The class is designed to be used as a callable, taking the current validation loss as an argument,
    and updating its internal state accordingly.
    Implemented from https://debuggercafe.com/using-learning-rate-scheduler-and-early-stopping-with-pytorch/ and
    modified.
    """
...
class LearningRateFinder:
    """Learning rate finder for determining an appropriate learning rate range.

    This class changes the learning rate of the model within a given range and factor and records the losses
    for each training iteration with the corresponding learning rate. The recorded data can be used to analyze
    and select a suitable learning rate for training.

    Parameters
    ----------
    trainer : Trainer
        The trainer object responsible for training the model.
    ...
    trainer : Trainer
        The trainer object responsible for training the model.
    lrs : list
        A list to store the learning rates used during the search.
    losses : list
        A list to store the losses corresponding to each learning rate.

    Methods
    -------
    find(self, min_lr=10e-10, max_lr=1e+1, lr_factor=1e+1)
        Search for the best learning rates between the given range and factor.

    Notes
    -----
    The class is designed to help in determining an appropriate learning rate range by plotting the loss
    curve against the learning rates. It helps identify a range where the loss decreases steeply before
    increasing again.
    """
    ...
    def __init__(self, trainer):
        """
        Initialize the LearningRateFinder object.
    ...
    def find(self, min_lr=10e-10, max_lr=1e+1, lr_factor=1e+1):
        """Search for the best learning rates between the given learning rate range and factor.

        Parameters
        ----------
        min_lr : float, optional
            The minimum learning rate of the search range. Default is 10e-10.
        max_lr : float, optional
            The maximum learning rate of the search range. Default is 1e+1.
        lr_factor : float, optional
            The factor by which the learning rate is multiplied at each step. Default is 1e+1.
        """
    ...
    def __plot_loss(self):
        """Plot the loss curve against the learning rates.
        """
